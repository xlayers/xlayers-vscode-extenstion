...
export class StyleOptimizerService {
    private static _instance: StyleOptimizerService;
    static getInstance() {
        if (!StyleOptimizerService._instance) {
            StyleOptimizerService._instance = new StyleOptimizerService();
        }
        return StyleOptimizerService._instance;
    }
     ...
     * @param ast any the ast based on sketch json
     */
    parseStyleSheet(ast: any) {
     ...
     */
    private buildAstStyleSheet(styles: StyleList[], ast: any): void {
        (function computeStyle(_ast: any, _styles) {
    ...
    private reduceDuplicates(duplicates: { className: string, key: string }[], stylesAst: StyleList[]) {
        const deDuplicateCssValues: Object = duplicates.reduce((current, next, index, _array) => {
            if (index === 0 || !current.hasOwnProperty(next.className)) {
                ((current as any)[next.className] as any) = {
                    className: next.className,
                    declarations: [next.key]
                };
            } else {
                (current as any)[next.className].declarations = [
                    ...(current as any)[next.className].declarations,
                    ...[next.key]
                ];
            }
